...
Функция ОбновлениеРасширение() Экспорт
	...
	ВременныйКаталогБазы = КаталогВременныхФайлов() + "Base" + Новый УникальныйИдентификатор();
	КаталогРасширения(ВременныйКаталогБазы, РезультатОбновления.ТекстОшибки);
	
	ИмяРасширения = "ПользовательскиеРоли";
	КаталогВыгрузки = ВременныйКаталогБазы + ПолучитьРазделительПутиСервера();
	ПутьКРасширению = КаталогВыгрузки + ИмяРасширения + ".cfe";
	ИспользоватьПромежуточнуюБазу = (Константы.гпр_ФормироватьРасширениеРолейЧерезПустуюБазу.Получить() = Истина);
	
	Если ИспользоватьПромежуточнуюБазу Тогда
	...
	КодВозвратаКомандыЗагрузкиВБазу = Неопределено;
	ТекстОшибки = "";
	СтрокаКоманды = КомандаЗагрузкиРасширенияВБазу(
		ВременныйКаталогБазы,
		ИмяРасширения,
		ИспользоватьПромежуточнуюБазу,
		ТекстОшибки
	);
	Если Не ПустаяСтрока(ТекстОшибки) Тогда
		РезультатОбновления.ТекстОшибки = ТекстОшибки;
		РезультатОбновления.Успешно = Ложь;
		Возврат РезультатОбновления;
	КонецЕсли;
	ЗапуститьПриложение(
		СтрокаКоманды, ,
		Истина,
		КодВозвратаКомандыЗагрузкиВБазу);
		
	Если КодВозвратаКомандыЗагрузкиВБазу <> 0 Тогда
			
		ЧтениеФайла = Новый ЧтениеТекста();
		ЧтениеФайла.Открыть(КаталогВыгрузки + "Report.txt");
		РезультатОбновления.ТекстОшибки = ЧтениеФайла.Прочитать();
		ЧтениеФайла.Закрыть();
		Возврат РезультатОбновления;
		
	КонецЕсли;		
	
	КодВозвратаКомандыЗагрузкиИзБазы = Неопределено;
	ТекстОшибки = "";
	СтрокаКоманды = КомандаВыгрузкиРасширенияИзБазы(
		ВременныйКаталогБазы,
		ИмяРасширения,
		ИспользоватьПромежуточнуюБазу,
		ТекстОшибки
	...
	ЗапуститьПриложение(
		СтрокаКоманды, ,
		Истина,
		КодВозвратаКомандыЗагрузкиИзБазы);
		
	Если КодВозвратаКомандыЗагрузкиИзБазы <> 0 Тогда
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			"Генерация расширения.Удаление файлов", УровеньЖурналаРегистрации.Ошибка,
			,
			,
	...
	КонецПопытки;
	гпр_ОбщегоНазначения.ЗаписатьОперациюБизнесСтатистики("УстановкаРасширенияПользовательскихРолей");
...
Процедура ДобавитьОбъектВФайл(ТекущийОбъект, КаталогРасширения, ШаблонОбъекта, ВерсияФорматаВыгрузки)
	...
	СвойстваОбъекта = ОсновныеСвойстваОбъекта(ДокументDOM, ТекущийОбъект.Объект, ТекущийОбъект.ВидОбъекта);
...
Функция ОсновныеСвойстваОбъекта(Знач ДокументDOM, ИмяСвойства, ВидОбъекта = Неопределено)
	...
	Если ВидОбъекта = Перечисления.гпр_ВидыМетаданных.ОбщаяФорма Тогда
		ПринадлежностьФормы = ДокументDOM.СоздатьЭлемент("FormType");
		ПринадлежностьФормы.ТекстовоеСодержимое = "Managed";
	КонецЕсли;
	...
	Если ВидОбъекта = Перечисления.гпр_ВидыМетаданных.ОбщаяФорма Тогда
		Свойства.ДобавитьДочерний(ПринадлежностьФормы);
	КонецЕсли;
...
Функция ЭлементыОбъектаПоВиду(ВидОбъекта)
	...
	ЭлементыОбработки.Добавить(Перечисления.гпр_ТипыРеквизитовОбъекта.ТабличнаяЧасть);
	...
	ЭлементыОтчета.Добавить(Перечисления.гпр_ТипыРеквизитовОбъекта.ТабличнаяЧасть);
	...
	ЭлементыПланВидов.Добавить(Перечисления.гпр_ТипыРеквизитовОбъекта.ТабличнаяЧасть);
...
Процедура ДополнитьРасширениеРолями(КаталогРасширения)
	...
	Пока ВыборкаПодсистемыРолей.Следующий() Цикл
		...
		Пока ВыборкаРолей.Следующий() Цикл
			...
			// Заполним роли подсистемам, которые настроены
			ПоискПравДоступа = Новый Структура("Ссылка", ВыборкаРолей.Ссылка);
			Если Не ВыборкаПравДоступаРолей.НайтиСледующий(ПоискПравДоступа) Тогда
				Продолжить;
			КонецЕсли;
			ВыборкаПравДоступаРолей.Сбросить();
...
Функция ТекстЗапросаПолученияРолей()
	...
	|	гпр_РолиПодсистемы.Ссылка КАК Ссылка,
...
Функция КомандаЗагрузкиРасширенияВБазу(КаталогБазы, ИмяРасширения, ИспользоватьПустуюБазу = Истина, ТекстОшибки = "")
	
	РазделительПутиСервера = ПолучитьРазделительПутиСервера();
	ШаблонКоманды = Новый Массив();
	ПутьКИсполняемомуФайлу = ПутьКИсполняемомуФайлу(ТекстОшибки);
	Если Не ПустаяСтрока(ТекстОшибки) Тогда
		Возврат "";
	КонецЕсли;
	ШаблонКоманды.Добавить(СтрШаблон("""%1""", ПутьКИсполняемомуФайлу));
	ДополнитьБазуРасширения(ШаблонКоманды, КаталогБазы, ИспользоватьПустуюБазу);
...
Процедура ДополнитьБазуРасширения(ШаблонКоманды, КаталогБазы, ИспользоватьПустуюБазу)

	Если ИспользоватьПустуюБазу Тогда
	...
	Иначе
		...
		Если СтрНачинаетсяС(ПутьБазы, "File=") Тогда
			...
			// База файловая
			ШаблонКоманды.Добавить(СтрШаблон("designer /f ""%1""", Сред(ПутьБазы, 7, СтрДлина(ПутьБазы) - 8)));
			
		ИначеЕсли СтрНачинаетсяС(ПутьБазы, "Srvr=") Тогда
			
			// База серверная
			ЧастиПути = СтрРазделить(СтрокаСоединенияИнформационнойБазы(), """;");
			ШаблонКоманды.Добавить(СтрШаблон("designer /s %1\%2", ЧастиПути[1], ЧастиПути[4]));
		КонецЕсли;
		ИмяПользователя = Константы.гпр_ПользовательБазы.Получить();
		ПарольПользователя = Константы.гпр_ПарольПользователя.Получить();
		
		Если Не ПустаяСтрока(ИмяПользователя) Тогда
			
			ШаблонКоманды.Добавить(СтрШаблон("/N""%1""", ИмяПользователя));
			
			Если Не ПустаяСтрока(ПарольПользователя) Тогда
				
				ШаблонКоманды.Добавить(СтрШаблон("/P""%1""",ПарольПользователя));
...
Функция ПутьКИсполняемомуФайлу(ТекстОшибки)
	
	КаталогПрограммы = Константы.гпр_КаталогДистрибутива1С.Получить();
	Если ЗначениеЗаполнено(КаталогПрограммы) Тогда
		КаталогПрограммы = КаталогПрограммы + "\";
	Иначе
		КаталогПрограммы = КаталогПрограммы();
	КонецЕсли;
	
	ПутьКИсполняемомуФайлу = КаталогПрограммы + ?(ЭтоWindows(), "1cv8.exe", "1cv8");
	ИсполняемыйФайл = Новый Файл(ПутьКИсполняемомуФайлу);
	Если Не ИсполняемыйФайл.Существует() Тогда
		СистемнаяИнформация = Новый СистемнаяИнформация;
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'На сервере не установлен дистрибутив ""Толстого клиента 1С"" версии %1. Получение сведений о конфигурации и формирование расширения невозможно.'"),
...
Функция КомандаВыгрузкиРасширенияИзБазы(КаталогБазы, ИмяРасширения, ИспользоватьПустуюБазу = Истина, ТекстОшибки = "")
