...
Функция ФайлыКонфигурации(ТекстОшибки, ИмяОбъекта = Неопределено, ИмяРасширения = "")
	...
	ИначеЕсли РежимВыгрузкиКонфигурации = Перечисления.гпр_РежимыВыгрузкиКонфигурации.КонфигурацияНеВыгруженаВКаталог Тогда
		...
		СтрокаКоманды = КомандаВыгрузитьКонфигурацию(ПараметрыКомандыВыгрузитьКонфигурацию, ТекстОшибки);
...
Функция КомандаВыгрузитьКонфигурацию(ПараметрыКоманды, ТекстОшибки)
...
Функция ПутьКИсполняемомуФайлу(ТекстОшибки)
	...
	Если ЗначениеЗаполнено(КаталогПрограммы) Тогда
		КаталогПрограммы = КаталогПрограммы + ПолучитьРазделительПутиСервера();
	Иначе
