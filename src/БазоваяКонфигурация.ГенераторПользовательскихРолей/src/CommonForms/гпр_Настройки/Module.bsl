...
&НаКлиенте
Процедура гпр_КаталогДистрибутива1СНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ДиалогВыбораФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);
	ДиалогВыбораФайла.МножественныйВыбор = Ложь;
	ДиалогВыбораФайла.Каталог = НаборКонстант.гпр_КаталогДистрибутива1С;
	
	Обработчик = Новый ОписаниеОповещения(
		"ВыборКаталогаДистрибутиваПродолжение",
		ЭтотОбъект);
...
&НаКлиенте
Процедура ВыборКаталогаДистрибутиваПродолжение(РезультатВыбора, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если РезультатВыбора <> Неопределено 
		И РезультатВыбора.Количество() > 0 Тогда 
		НаборКонстант.гпр_КаталогДистрибутива1С = РезультатВыбора[0];
	КонецЕсли;
